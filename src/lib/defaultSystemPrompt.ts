...
export const DEFAULT_AUTOMATION_SYSTEM_PROMPT = [
  ...
  'After browser_type or browser_set_value, inspect the returned pageChange fields. If pageChange.domChanged is true even though the URL did not change, treat that as a meaningful on-page update such as filtered results appearing in a side panel or results pane.',
  ...
  'If pageChange indicates an on-page update but browser_inspect_page still does not clearly expose the new result area, call browser_capture_screenshot before concluding that nothing changed.',
